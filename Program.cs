...
{
    ulong number = 0;   // По хорошему тут надо бы присвоить -1, но тогда придется использовать long вместо ulong,
    while(true)         // а это значит, что число будет иметь на 12 разрядов меньше
    {
        ...
        catch (System.Exception)       // тут наверно лучше использовать конкретный Эксепшн наследник этого, но я не разобрался как это работает в C#
        {
...
{                                           // программы за одну итерацию по циклу нечётные числа 
    ...
    {
        flag = false;
        for (int i = 1; i < degree; i++)    // В условиях проверяем два рядомстоящих числа, и если что меняем их местами
        {
